...
internal partial class Folder : ProjectItemContent, IProjectItemContentCreatable<Folder> {
    ...
    public static async Task<Folder> Create(ProjectPath file, ProjectViewModel project) {
        await file.ToStorageFolder(project);
        return new Folder();
    }

    public static async Task<Folder> Load(ProjectPath path, ProjectViewModel project) {
